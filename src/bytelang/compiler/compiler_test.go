...
func TestStringExpressions(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input:             `"monkey" + "key"`,
			expectedConstants: []interface{}{"monkey", "key"},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpAdd),
...
func TestCompilerScopes(t *testing.T) {
	...
	if compiler.symbolTable.Enclosing != globalSymbolTable {
	...
	if compiler.symbolTable.Enclosing != nil {
	...
	previous := compiler.scopes[compiler.scopeIndex].penultimateInstruction
...
func testStringObject(constant string, o object.Object) error {
	result, ok := o.(*object.String)
	if !ok {
		return fmt.Errorf("object is not of type String. got %T", result)
	}

	if result.Value != constant {
		return fmt.Errorf("expected value %s. got %s", constant, result.Value)
	}
